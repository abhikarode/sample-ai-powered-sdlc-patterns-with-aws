...
exports.AdvancedRAGConfig = void 0;
class AdvancedRAGConfig {
     ...
     */
    getHybridSearchConfig(complexity) {
     ...
     */
    extractEnhancedSources(response) {
    ...
    validateResponseQuality(response) {
        ...
        if (reliabilityScore < 0.5)
            warnings.push('Low confidence in sources');
        if (coherenceScore < 0.5)
            warnings.push('Response may lack coherence');
        return {
    ...
    extractMetadataString(metadata, key, defaultValue = '') {
        const value = metadata[key];
        return typeof value === 'string' ? value : defaultValue;
    }
    calculateRelevanceScore(reference, citation) {
    ...
    calculateContextualRelevance(reference) {
        ...
        if (text.length > 500)
            score += 0.1; // Comprehensive content
        return Math.min(score, 1.0);
    }
    extractKeywordMatches(text) {
        // Simple keyword extraction (in production, use more sophisticated NLP)
        const words = text.toLowerCase().match(/\b\w{4,}\b/g) || [];
        const uniqueWords = [...new Set(words)];
        return uniqueWords.slice(0, 10); // Top 10 keywords
    }
    calculateCompletenessScore(answer) {
        ...
        if (answer.includes(':') || answer.includes('-'))
            score += 0.05; // Lists or explanations
        if (answer.match(/\b(because|therefore|however|additionally)\b/i))
            score += 0.05; // Reasoning words
        return Math.min(score, 1.0);
    }
    ...
    calculateQuerySpecificRelevance(source, query) {
...
exports.AdvancedRAGConfig = AdvancedRAGConfig;
//# sourceMappingURL=advanced-rag-config.js.map
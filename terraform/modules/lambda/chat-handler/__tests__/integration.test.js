...
describe('Chat Handler Lambda - Integration Tests (Real AWS)', () => {
    ...
    describe('CORS Handling (Real AWS)', () => {
        test('should handle OPTIONS preflight request', async () => {
            ...
            const result = await (0, index_1.handler)(optionsEvent, mockContext);
    ...
    describe('Error Handling (Real AWS)', () => {
        test('should handle invalid JSON in request body', async () => {
            ...
            const result = await (0, index_1.handler)(eventWithInvalidJson, mockContext);
            expect(result.statusCode).toBe(400);
            const errorResponse = JSON.parse(result.body);
    ...
    describe('Model Selection Logic (Real AWS)', () => {
        test('should handle different query complexities', async () => {
            const testCases = [
                {
                    question: 'Hi',
                    expectedComplexity: types_1.QueryComplexity.SIMPLE
                },
                {
                    question: 'What is AWS Lambda and how does it work?',
                    expectedComplexity: types_1.QueryComplexity.MODERATE
                },
                {
                    ...
                    expectedComplexity: types_1.QueryComplexity.COMPLEX
                }
            ];

...
describe('Advanced RAG Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        const testDocument = (0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../sample-documents/advanced-rag-test.md'), 'utf-8');
    ...
    describe('Hybrid Search Configuration', () => {
        test('should use hybrid search for moderate complexity queries', async () => {
            ...
            const request = {
                ...
                userId: 'test-user',
                queryComplexity: types_1.QueryComplexity.MODERATE
            };
        ...
        test('should retrieve relevant sources with confidence scores', async () => {
            ...
            const request = {
                ...
                userId: 'test-user',
                queryComplexity: types_1.QueryComplexity.SIMPLE
            };
            const response = await bedrockService.handleChatQueryWithAdvancedRAG(request);
            ...
            if (response.sources.length > 0) {
                ...
                for (let i = 1; i < response.sources.length; i++) {
                    expect(response.sources[i - 1].confidence).toBeGreaterThanOrEqual(response.sources[i].confidence);
                }
            }
            else {
    ...
    describe('Response Quality Validation', () => {
        test('should provide quality metrics for responses', async () => {
            ...
            const request = {
                ...
                userId: 'test-user',
                queryComplexity: types_1.QueryComplexity.COMPLEX
            };
            const response = await bedrockService.handleChatQueryWithAdvancedRAG(request);
            expect(response.qualityMetrics).toBeDefined();
...
//# sourceMappingURL=advanced-rag-integration.test.js.map
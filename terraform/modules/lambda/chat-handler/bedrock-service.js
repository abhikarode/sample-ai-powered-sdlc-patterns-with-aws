...
const advanced_rag_config_1 = require("./advanced-rag-config");
...
class BedrockService {
    constructor() {
        ...
        this.bedrockAgentRuntime = new client_bedrock_agent_runtime_1.BedrockAgentRuntimeClient(clientConfig);
        this.bedrockRuntime = new client_bedrock_runtime_1.BedrockRuntimeClient(clientConfig);
        this.cloudWatch = new client_cloudwatch_1.CloudWatchClient(clientConfig);
        this.advancedRAG = new advanced_rag_config_1.AdvancedRAGConfig();
    ...
    async handleChatQuery(request) {
        const startTime = Date.now();
        try {
            // Classify query complexity if not provided
            const queryComplexity = request.queryComplexity ||
                (0, model_config_1.classifyQueryComplexity)(request.question);
            // Select optimal model
            const selectedModel = (0, model_config_1.selectOptimalModel)(queryComplexity);
            ...
            const response = await this.retrieveAndGenerate(request.question, availableModel
            // Don't pass conversationId as sessionId - let Bedrock manage sessions
            );
            // Calculate token usage and cost
            const tokenUsage = this.extractTokenUsage(response);
            const modelConfig = (0, model_config_1.getModelConfigByName)(availableModel);
            ...
            await this.trackKnowledgeBaseQueryMetrics(request.question, request.userId || 'unknown', Date.now() - startTime, true, response.citations?.length || 0);
            return {
                answer: response.output?.text || 'No response generated',
                sources: this.extractSources(response),
                conversationId: response.sessionId || request.conversationId || (0, uuid_1.v4)(),
    ...
    async handleChatQueryWithAdvancedRAG(request) {
        const startTime = Date.now();
        try {
            // Classify query complexity if not provided
            const queryComplexity = request.queryComplexity ||
            ...
            const response = await this.retrieveAndGenerateAdvanced(request.question, availableModel, ragConfig
            // Don't pass conversationId as sessionId - let Bedrock manage sessions
            );
            ...
            const filteredSources = this.advancedRAG.filterByRelevance(enhancedSources, ragConfig.qualityThresholds.minConfidence);
            const rankedSources = this.advancedRAG.enhanceSourceRanking(filteredSources, request.question);
            ...
            await this.trackAdvancedMetrics(availableModel, tokenUsage, Date.now() - startTime, qualityMetrics, ragConfig);
            return {
    ...
    async retrieveAndGenerate(question, modelArn) {
    ...
    async retrieveAndGenerateAdvanced(question, modelArn, ragConfig) {
        const knowledgeBaseId = this.getKnowledgeBaseId();
    ...
    calculateCost(tokenUsage, modelConfig) {
        if (!modelConfig) {
            return 0;
        }
    ...
    async trackMetrics(modelUsed, tokenUsage, responseTime) {
        try {
    ...
    buildAdvancedPromptTemplate(ragConfig) {
        return `
...
Please provide a detailed, well-structured answer:
    ...
    async trackAdvancedMetrics(modelUsed, tokenUsage, responseTime, qualityMetrics, ragConfig) {
        try {
            const command = new client_cloudwatch_1.PutMetricDataCommand({
    ...
    async trackKnowledgeBaseQueryMetrics(question, userId, responseTime, success, sourcesFound) {
        try {
            const command = new client_cloudwatch_1.PutMetricDataCommand({
    ...
    handleBedrockError(error) {
        const bedrockError = new Error(error.message);
